...
import MultipleGalleryItems from "./MultipleGalleryItems";
                ...
                <div className="flex flex-col h-full pt-10 pb-20">
                    ...
                    {selected && <div className="flex-1 relative">                                
                        ...
                        <div className="absolute top-0 left-0 w-full h-full flex gap-x-14 items-start">
                            <div className={isStrech ? 'hidden' : 'w-4/12'}>
                            ...
                            <div className="h-full mx-auto relative">
                                ...
                                {selected?.images ?
                                    <MultipleGalleryItems isVideo={selected.is_video} images={selected.images} isStrech={isStrech} /> :
                                    <SimpleGalleryItems isVideo={isVideo} selected={selected} />
                                } 
                            ...
                            <div className={isStrech ? 'hidden' : 'w-4/12 max-h-full overflow-auto'}>
                                {selected?.description[i18n.resolvedLanguage as 'hu' | 'en'].map((item, i) => {
                                    return <p key={i} className="text-fuscous-grayd text-2xl">{item}</p>
                                })}
                            </div>
                        </div>
